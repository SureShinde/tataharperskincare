...
class Magentostudy_News_Adminhtml_NewsController extends Mage_Adminhtml_Controller_Action
{
    ...
    protected function _initAction()
    {
        ...
        $this->loadLayout()
            ...
            ->_addBreadcrumb(
                  Mage::helper('magentostudy_news')->__('Press Page'),
                  Mage::helper('magentostudy_news')->__('Press Page')
              )
            ->_addBreadcrumb(
                  Mage::helper('magentostudy_news')->__('Manage Press Page'),
                  Mage::helper('magentostudy_news')->__('Manage Press Page')
              )
        ;
        return $this;
    ...
    {
        $this->_title($this->__('Press Page'))
             ->_title($this->__('Press Page'));
    ...
    {
        $this->_title($this->__('Press Page'))
             ->_title($this->__('Manage Press Page'));
    ...
    public function saveAction()
    {
        ...
        if ($data) {
            ...
            if (isset($data['image_main'])) {
                $imageDataMain = $data['image_main'];
                unset($data['image_main']);
            } else {
                $imageDataMain = array();
            }
             if (isset($data['image_popup'])) {
                $imageDataPopup = $data['image_popup'];
                unset($data['image_popup']);
            } else {
                $imageDataPopup = array();
            }
            $model->addData($data);
            ...
            try {
                ...
                if (isset($imageData['delete']) && $model->getImageMain()) {
                    $imageHelper->removeImage($model->getImageMain());
                    $model->setImageMain(null);
                }

                if (isset($imageData['delete']) && $model->getImagePopup()) {
                    $imageHelper->removeImage($model->getImagePopup());
                    $model->setImagePopup(null);
                }

                // upload new image
                $imageFile = $imageHelper->uploadImage('image_main');
                if ($imageFile) {
                    if ($model->getImageMain()) {
                        $imageHelper->removeImage($model->getImageMain());
                    }
                    $model->setImageMain($imageFile);
                }
                $imageFile = $imageHelper->uploadImage('image_popup');
                if ($imageFile) {
                    if ($model->getImagePopup()) {
                        $imageHelper->removeImage($model->getImagePopup());
                    }
                    $model->setImagePopup($imageFile);
                }
                // save the data
                $model->save();
    ...
    public function deleteAction()
    {
        ...
        if ($itemId) {
            try {
                ...
                $model->load($itemId);
                if (!$model->getId()) {
                    Mage::throwException(Mage::helper('magentostudy_news')->__('Unable to find a press item.'));
                }
                $model->delete();

                // display success message
                $this->_getSession()->addSuccess(
                    Mage::helper('magentostudy_news')->__('The press item has been deleted.')
                );
            } catch (Mage_Core_Exception $e) {
                $this->_getSession()->addError($e->getMessage());
            } catch (Exception $e) {
                $this->_getSession()->addException($e,
                    Mage::helper('magentostudy_news')->__('An error occurred while deleting the press item.')
                );
            }
        }
