...
class ParadoxLabs_AuthorizeNetCim_Adminhtml_AuthnetcimController extends Mage_Adminhtml_Controller_Action
{
	...
	public function saveAction() {
		...
		if( count($payment) && $key == Mage::getSingleton('core/session')->getFormKey() ) {
			try {
				if( $card > 0 ) {
					Mage::getModel('authnetcim/payment')->updateCustomerPaymentProfile( $card, $payment, $customer->getAuthnetcimProfileId() );
				}
				else {
					Mage::getModel('authnetcim/payment')->createCustomerPaymentProfileFromForm( $payment, $customer->getAuthnetcimProfileId() );
				}
			}
	...
	public function deleteAction() {
		...
		if( $card > 0 && $key == Mage::getSingleton('core/session')->getFormKey() ) {
			Mage::getModel('authnetcim/payment')->deletePaymentProfile( $card, $customer->getAuthnetcimProfileId(), false );
		}
