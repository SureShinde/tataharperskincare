...
// Mage::log('Loaded Authorize.net CIM module: '.date('m-d-Y h:i:s-u'), null, 'authnetcim.log');
	...
	implements Mage_Payment_Model_Recurring_Profile_MethodInterface
{
	...
	public function runDailyBilling() {
		...
		if( Mage::getModel('authnetcim/payment')->getConfigData('active') == 0 ) {
			return;
		}
				...
				if( isset($adtl['next_cycle']) && $adtl['next_cycle'] <= time() ) {
					...
					if( $price > 0 ) {
						try {
							...
							$order->setState( Mage::getModel('authnetcim/payment')->getConfigData('order_status') )
								  ->setStatus( Mage::getModel('authnetcim/payment')->getConfigData('order_status') )
								  ->setExtCustomerId( $adtl['payment_id'] )
								  ->save();
							...
							Mage::dispatchEvent( 'sales_order_place_after', array( 'order' => $order, 'profile' => $profile ) );
							...
							if( $profile->getSuspensionThreshold() != null && $adtl['failure_count'] >= $profile->getSuspensionThreshold() ) {
								...
								Mage::log( "Recurring profile #{$refId} failed on payment ({$adtl['failure_count']}/{$profile->getSuspensionThreshold()}); profile terminated.", null, 'authnetcim.log', true );
	...
	public function validate() {
		...
		if( empty($_POST['payment']['payment_id']) || !empty($_POST['payment']['cc_number']) ) {
			try {
	...
	public function void(Varien_Object $payment) {
		...
		try {
			...
			$payment->getOrder()->setState( Mage_Sales_Model_Order::STATE_CANCELED )
	...
	public function isAvailable($quote=null) {
		return (bool)($this->getConfigData('active') || $this->_admin);
	}
		...
		if( strtotime($profile->getStartDatetime()) <= time() ) {
			...
			if( $price > 0 ) {
				try {
					...
					$order = $profile->createOrder( $productItemInfo );
					$order->setState( $this->getConfigData('order_status') )
						  ->setStatus( $this->getConfigData('order_status') )
						  ->setExtCustomerId( $payment_id )
						  ->save();
					...
					if( !$profile->getInitMayFail() || $profile->getSuspensionThreshold() === 0 ) {
						...
						Mage::log( "Recurring profile #{$refId} failed on initial payment.", null, 'authnetcim.log', true );
						Mage::log( $e->getMessage(), null, 'authnetcim.log', true );
	...
	public function createCustomerPaymentProfileFromForm( $input, &$profile_id=0 ) {
		...
		try {
			...
			return $this->cim->getCustomerAddressId();
		}
		catch (AuthnetCIMException $e) {
			Mage::log( $e->getMessage(), null, 'authnetcim.log', true );
		}
	...
	protected function createCustomerPaymentProfile(&$_customer, &$payment, $profile_id=0) {
		...
		try {
			...
			if( $this->cim->getCode() == 'E00040' ) {
				$profile_id = $this->createCustomerProfile( $_customer, $payment );
				return $this->createCustomerPaymentProfile( $_customer, $payment, $profile_id );
			}

			if( intval($billing_id) <= 0 ) {
				Mage::log( 'Unable to get/create payment ID.', null, 'authnetcim.log', true );
				Mage::log( 'CIM: '.$this->cim->responses, null, 'authnetcim.log', true );
				Mage::throwException( "Authorize.net CIM Gateway: Payment failed. \nPlease verify your payment details and try again in three minutes. \nIf you continue having problems, please seek support." );
			}
	...
	public function updateCustomerPaymentProfile(&$billing_id, &$data, &$profile_id=0) {
		...
		try {
			if( $profile_id < 1 ) {
				$profile_id = $this->getProfileId( $this->getCustomer(), $payment );
			}
	...
	protected function createCustomerPaymentProfileRecurring(&$_customer, &$profile, $profile_id=0) {
		...
		try {
			...
			if( intval($billing_id) <= 0 ) {
				Mage::log( 'Unable to get/create payment ID.', null, 'authnetcim.log', true );
				Mage::log( 'CIM: '.$this->cim->responses, null, 'authnetcim.log', true );
				Mage::throwException( "Authorize.net CIM Gateway: Payment failed; unable to create a payment ID. Please seek support." );
			}
	...
	protected function bill( &$payment, $amount, $type = 'profileTransAuthOnly' ) {
		...
		try {
			...
			if( $payment->getTaxAmount() )
				$this->cim->setParameter( 'taxAmount', $payment->getTaxAmount() );
			
			if( $payment->getShippingAmount() )
				$this->cim->setParameter( 'shipAmount', $payment->getShippingAmount() );
	...
	protected function getCimResponse() {
		...
		$r 		= explode( ',', str_replace('"','',$this->cim->getDirectResponse()) );
	 ...
	 */
	protected function checkCimErrors() {
