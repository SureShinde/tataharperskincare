...
abstract class Mage_Sales_Model_Order_Pdf_Abstract extends Varien_Object
{
    ...
    protected function insertOrder(&$page, $obj, $putOrderId = true)
    {
        ...
        $page->drawText(Mage::helper('sales')->__('Sold to:') . $order->getCustomerEmail(), 35, ($top - 15), 'UTF-8');
        ...
        if ($order->getIsVirtual()) {
            ...
            $yShipments -= $topMargin + 1;

            /* AX 11/12/13 -- Add custom fields */
            // 1. Customer Notes
            $cModel = Mage::getModel('custom/custom_order');
            $cData = $cModel->getByOrder($order->getId());
            foreach($cData as $key => $value){
                if ($key == "comments")
                {
                    $comments = str_split($value, 45);
                    foreach ($comments as $comment) {
                         $page->drawText("Order comments: ", 285, $yShipments - $topMargin, 'UTF-8');
                         $yShipments -= $topMargin + 1;
                         $page->drawText($comment, 285, $yShipments - $topMargin, 'UTF-8');
                         $yShipments -= $topMargin + 1;
                    }
                }
            }
            ...
            if ($transactionId == "")
                $transactionId = $orderData['additional_information']['paypal_payment_status'];

            $page->drawText("Payment verification: ", 285, $yShipments - $topMargin, 'UTF-8');
            $yShipments -= $topMargin + 1;

            $page->drawText("PTI2: " . $transactionId, 285, $yShipments - $topMargin, 'UTF-8');
            $yShipments -= $topMargin + 1;
            ...
            if(!is_null($gift_message_id)) {
                ...
                $page->drawText("Gift message: ", 285, $yShipments - $topMargin, 'UTF-8');
                $yShipments -= $topMargin + 1;

                $page->drawText("Sender: " . $gift_sender, 285, $yShipments - $topMargin, 'UTF-8');
                $yShipments -= $topMargin + 1;

                $page->drawText("Recipient: " . $gift_recipient, 285, $yShipments - $topMargin, 'UTF-8');
                $yShipments -= $topMargin + 1;

                $page->drawText("Message: " . $gift_message, 285, $yShipments - $topMargin, 'UTF-8');
            }
            ...
            // $currentY = min($yPayments, $yShipments);
             $currentY = min($yPayments, $yShipments) - 75; // TEST AX
