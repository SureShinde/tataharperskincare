...
$installer->run("
        ALTER TABLE `{$orderTable}` ADD `status_preorder` VARCHAR (50) NOT NULL COMMENT 'Status Preorder' AFTER `status`;
...
if(version_compare(Mage::getVersion(),'1.4.1','>='))
{
	...
	$installer->run("
		ALTER TABLE `{$orderGridTable}` ADD `status_preorder` VARCHAR (50) NOT NULL COMMENT 'Status Preorder' AFTER `status`;
...
{
        $orderGridArchiveTable = $installer->getTable('enterprise_salesarchive/order_grid');
	$installer->run("
		ALTER TABLE `{$orderGridArchiveTable}` ADD `status_preorder` VARCHAR (50) NOT NULL COMMENT 'Status Preorder' AFTER `status`;
	");
}
